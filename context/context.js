...
export const SwapContextProvider = ({children}) => {
    ...
    const [toCoinModal, setToCoinModal] = useState(false)
    ...
    useEffect(() => {
        if (isAuthenticated) {
            ...
            const formatedAccount= account.slice(0,4)+'...'+account.slice(-4)
    ...
    return (
        <SwapContext.Provider value={{logOut, setPinnedToken, pinnedTokens, usdPrice, doSwap, coinSelectModal, toCoinAmount, estimatedGas, setCoinSelectModal, toCoinModal, setToCoinModal, tokens, login, currentAccount, formatedAccount, isAuthenticated, coinSelect, setCoinSelect, toCoin, setToCoin, amount, setAmount}}>
            {children}
